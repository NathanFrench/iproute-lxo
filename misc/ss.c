...
static int
getpidcon(pid_t pid, char ** context)
{
    *context = NULL;
    return -1;
}
...
static int
getfilecon(char * path, char ** context)
{
    *context = NULL;
    return -1;
}
...
static int
security_get_initial_context(char * name, char ** context)
{
    *context = NULL;
    return -1;
}
...
#define SS_CONN      (SS_ALL &                                                     \
                      ~((1 << SS_LISTEN) | (1 << SS_CLOSE) | (1 << SS_TIME_WAIT) | \
                        (1 << SS_SYN_RECV)))
...
static void
filter_db_set(struct filter * f, int db, bool enable)
{
    if (enable) {
...
static int
filter_db_parse(struct filter * f, const char * s)
{
    const struct {
        const char * name;
        int          dbs[MAX_DB + 1];
...
static void
filter_af_set(struct filter * f, int af)
{
...
static int
filter_af_get(struct filter * f, int af)
{
    return !!(f->families & FAMILY_MASK(af));
}

static void
filter_states_set(struct filter * f, int states)
{
    if (states) {
        f->states = states;
    }
...
static void
filter_merge_defaults(struct filter * f)
{
    int db;
    int af;
...
static FILE *
generic_proc_open(const char * env, const char * name)
{
...
static int
user_ent_hashfn(unsigned int ino)
{
...
static void
user_ent_add(unsigned int ino, char * process,
             int pid, int fd,
             char * proc_ctx,
             char * sock_ctx)
{
...
static int
find_entry(unsigned int ino, char ** buf, int type)
{
    struct user_ent * p;
    int cnt = 0;
    char * ptr;
        ...
        while (1) {
            ...
            switch (type) {
                case USERS:
                    ...
                    p->pid,
                    p->fd);
                    xo_close_instance("proc-info");
                    len = snprintf(ptr,
                    buf_len - buf_used,
                    "({:process/%s},pid={:pid/%d},fd={:fd/%d}),",
                    p->process,
                    p->pid,
                    p->fd);
                    break;
...
static int
emit_process_info(const char * name, unsigned int ino, int type)
{
    struct user_ent * p;
    int res = 0;
...
static int
get_slabstat(struct slabstat * s)
{
    char buf[256];
    FILE * fp;
    int cnt;
...
cookie_sk_get(const uint32_t * cookie)
{
 ...
 * endpoint. */
static bool
is_sctp_assoc(struct sockstat * s, const char * sock_name)
{
...
static int
buf_chunk_avail(struct buf_chunk * chunk)
{
...
static int
print_left_spacing(struct column * f, int stored, int printed)
{
    int s;
...
static void
print_right_spacing(struct column * f, int printed)
{
    int s;
...
static struct buf_token *
buf_token_next(struct buf_token * cur)
{
    struct buf_chunk * chunk = buffer.tail;
...
sock_state_print(struct sockstat * s)
{
    ...
    field_set(COL_RECVQ);
    /*out("%-6d", s->rq); */
    xo_emit("{:recv-queue/%-6d/%d}{P: }", s->rq);

    field_set(COL_SENDQ);
    /*out("%-6d", s->wq); */
    xo_emit("{:send-queue/%-6d/%d}{P: }", s->wq);

    field_set(COL_ADDR);
} /* sock_state_print */

static void
sock_details_print(struct sockstat * s)
{
    if (s->uid)
    {
        /*out(" uid:%u", s->uid); */
        xo_emit("{P: }{Lc:uid}{:uid-number/%u}", s->uid);
    }
     ...
     *
     */
    xo_emit("{P: }{Lc:ino}{:inode/%u}", s->ino);
    xo_emit("{P: }{Lc:sk}{:sock-cookie/%llx}", s->sk);

    if (s->mark)
    {
        xo_emit("{P: }{Lc:fwmark}{:fw-mark/0x%x}", s->mark);
...
static void
sock_addr_print(const char * addr, char * delim, const char * port,
                const char * ifname)
{
    if (ifname) {
...
static void
inet_addr_print(const inet_prefix * a, int port,
...
static int
inet2_addr_match(const inet_prefix * a, const inet_prefix * p,
                 int plen)
{
...
static int
unix_match(const inet_prefix * a, const inet_prefix * p)
{
    char * addr, * pattern;
...
static int
run_ssfilter(struct ssfilter * f, struct sockstat * s)
{
    switch (f->type) {
        case SSF_S_AUTO:
        {
...
static void
ssfilter_patch(char * a, int len, int reloc)
{
    while (len > 0) {
...
static int
ssfilter_bytecompile(struct ssfilter * f, char ** bytecode)
{
    switch (f->type) {
        case SSF_S_AUTO:
        {
            if (!(*bytecode = malloc(4))) { abort(); }
            ((struct inet_diag_bc_op *)*bytecode)[0] =
                (struct inet_diag_bc_op) {INET_DIAG_BC_AUTO, 4, 8 };
            return 4;
        }
        case SSF_DCOND:
        case SSF_SCOND:
        ...
        case SSF_D_GE:
        {
            ...
            ((struct inet_diag_bc_op *)*bytecode)[0] =
                (struct inet_diag_bc_op) {INET_DIAG_BC_D_GE, 8, 12 };
        ...
        case SSF_D_LE:
        {
            ...
            if (!(*bytecode = malloc(8))) { abort(); }
            ((struct inet_diag_bc_op *)*bytecode)[0] =
                (struct inet_diag_bc_op) {INET_DIAG_BC_D_LE, 8, 12 };
            ((struct inet_diag_bc_op *)*bytecode)[1] = (struct inet_diag_bc_op) {0, 0, x->port };
            return 8;
        }
        case SSF_S_GE:
        {
            ...
            if (!(*bytecode = malloc(8))) { abort(); }
            ((struct inet_diag_bc_op *)*bytecode)[0] =
                (struct inet_diag_bc_op) {INET_DIAG_BC_S_GE, 8, 12 };
            ((struct inet_diag_bc_op *)*bytecode)[1] = (struct inet_diag_bc_op) {0, 0, x->port };
            return 8;
        }
        case SSF_S_LE:
        {
            ...
            if (!(*bytecode = malloc(8))) { abort(); }
            ((struct inet_diag_bc_op *)*bytecode)[0] =
                (struct inet_diag_bc_op) {INET_DIAG_BC_S_LE, 8, 12 };
            ((struct inet_diag_bc_op *)*bytecode)[1] = (struct inet_diag_bc_op) {0, 0, x->port };
            return 8;
        }
        ...
        case SSF_OR:
        {
            ...
            memcpy(a + l1 + 4, a2, l2);
            free(a1); free(a2);
            *(struct inet_diag_bc_op *)(a +
            l1) = (struct inet_diag_bc_op) {INET_DIAG_BC_JMP, 4, l2 + 4 };
            *bytecode = a;
            return l1 + l2 + 4;
        }
        case SSF_NOT:
        {
            ...
            free(a1);
            *(struct inet_diag_bc_op *)(a +
            l1) = (struct inet_diag_bc_op) {INET_DIAG_BC_JMP, 4, 8 };
            *bytecode = a;
            return l1 + 4;
        }
        case SSF_DEVCOND:
...
static int
remember_he(struct aafilter * a, struct hostent * he)
{
...
static int
get_dns_host(struct aafilter * a, const char * addr, int fam)
{
    static int notfirst;
    int cnt = 0;
    struct hostent * he;
...
static int
xll_name_to_index(const char * dev)
{
    if (!xll_initted) {
        xll_init();
    }
...
static void
vsock_set_inet_prefix(inet_prefix * a, __u32 cid)
{
    *a = (inet_prefix) {
        .bytelen = sizeof(cid),
        .family  = AF_VSOCK,
...
void *
parse_hostcond(char * addr, bool is_port)
{
    char * port       = NULL;
...
void *
parse_markmask(const char * markmask)
{
...
static void
proc_ctx_print(struct sockstat * s)
{
    char * buf;
    int len;
    ...
    if (show_proc_ctx || show_sock_ctx) {
        ...
        (show_proc_ctx & show_sock_ctx) ?  PROC_SOCK_CTX : PROC_CTX) > 0) {
            out(" users:(%s)", buf);
            free(buf);
        }
#endif
    } else if (show_users) {
        xo_emit("{P: }{Lc:users}{D:(}");
        xo_open_list("proc-info");
        emit_process_info("proc-info", s->ino, USERS);
        xo_close_list("proc-info");
        xo_emit("{D:)}");
    }
}

static void
inet_stats_print(struct sockstat * s, bool v6only)
{
    sock_state_print(s);
...
static int
proc_parse_inet_addr(char * loc, char * rem, int family, struct
                     sockstat * s)
{
...
static int
proc_inet_split_line(char * line, char ** loc, char ** rem, char ** data)
{
    char * p;
...
static void
sctp_stats_print(struct sctp_info * s)
{
    if (s->sctpi_tag) {
...
static void
tcp_stats_print(struct tcpstat * s)
{
    char b1[64];

    if (s->has_ts_opt) {
        xo_emit("{P: }{L:ts}{en:timestamp/true}");
    }

    if (s->has_sack_opt) {
        xo_emit("{P: }{L:sack}{en:sack/true}");
    }

    if (s->has_ecn_opt) {
        xo_emit("{P: }{L:ecn}{en:explicit-congestion-notif/true}");
    }

    if (s->has_ecnseen_opt) {
        xo_emit("{P: }{L:ecnseen}{en:ecnseen/true}");
    }

    if (s->has_fastopen_opt) {
        xo_emit("{P: }fastopen{en:fastopen/true}");
    }

    if (s->cong_alg[0]) {
        xo_emit("{P: }{:congestion-algorithm-name}", s->cong_alg);
    }

    if (s->has_wscale_opt) {
        xo_emit("{P: }{Lc:wscale}{:snd-scale/%d},{:rcv-scale/%d}", s->snd_wscale, s->rcv_wscale);
    }

    if (s->rto) {
        xo_emit("{P: }{Lc:rto}{:retrans-timeout-ms/%g}", s->rto);
    }

    if (s->backoff) {
        xo_emit("{P: }{Lc:backoff}{:backoff/%u}", s->backoff);
    }

    if (s->rtt) {
        xo_emit("{P: }{Lc:rtt}{:round-trip-time/%g}/{:rtt-mdeviation/%g}", s->rtt, s->rttvar);
    }

    if (s->ato) {
        xo_emit("{P: }{Lc:ato}{:ack-timeout-ms/%g}", s->ato);
    }

    if (s->qack) {
        xo_emit("{P: }{Lc:qack}{:quick-ack/%d}", s->qack);
    }

    if (s->qack & 1) {
        xo_emit("{P: }{l:bidir}{en:bidir/true}");
    }

    if (s->mss) {
        xo_emit("{P: }{Lc:mss}{:mss/%d}", s->mss);
    }

    if (s->pmtu) {
        xo_emit("{P: }{Lc:pmtu}{:path-mtu/%u}", s->pmtu);
    }

    if (s->rcv_mss) {
        xo_emit("{P: }{Lc:rcvmss}{:rcv-mss/%d}", s->rcv_mss);
    }

    if (s->advmss) {
        xo_emit("{P: }{Lc:advmss}{:advmss/%d}", s->advmss);
    }

    if (s->cwnd) {
        xo_emit("{P: }{Lc:cwnd}{:cong-win-size/%u}", s->cwnd);
    }

    if (s->ssthresh) {
        xo_emit("{P: }{Lc:ssthresh}{:cong-win-slow-start-thresh/%d}", s->ssthresh);
    }

    if (s->bytes_acked) {
        xo_emit("{P: }{Lc:bytes_acked}{:bytes-acked/%llu}", s->bytes_acked);
    }

    if (s->bytes_received) {
        xo_emit("{P: }{Lc:bytes_received}{:bytes-received/%llu}", s->bytes_received);
    }

    if (s->segs_out) {
        xo_emit("{P: }{Lc:segs_out}{:segments-sent-out/%u}", s->segs_out);
    }

    if (s->segs_in) {
        xo_emit("{P: }{Lc:segs_in}{:segments-sent-in/%u}", s->segs_in);
    }

    if (s->data_segs_out) {
        xo_emit("{P: }{Lc:data_segs_out}{:data-segs-out/%u}", s->data_segs_out);
    }

    if (s->data_segs_in) {
        xo_emit("{P: }{Lc:data_segs_in}{:data-segs-in/%u}", s->data_segs_in);
    }
    ...
    if (s->dctcp && s->dctcp->enabled) {
        ...
        xo_emit(
            "{P: }{Lc:dctcp}({Lc:ce_state}{:ce-state/%u},{Lc:alpha}{:alpha/%u},{Lc:ab_ecn}{:ab-ecn/%u},{Lc:ab_tot}{:ab-tot/%u})",
            dctcp->ce_state,
        dctcp->alpha,
        dctcp->ab_ecn,
        dctcp->ab_tot);
    } else if (s->dctcp) {
        xo_emit("{P: }{Lc:dctcp}fallback_mode");
    }

    if (s->bbr_info) {
        __u64 bw;

        xo_open_container("bbr-info");
        bw   = s->bbr_info->bbr_bw_hi;
        bw <<= 32;
        bw  |= s->bbr_info->bbr_bw_lo;

        xo_emit("{P: }{Lc:bbr}({Lc:bw}{:bw}{N:bps},{Lc:mrtt}{:min-rtt-ms/%g}",
            sprint_bw(b1, bw * 8.0), (double)s->bbr_info->bbr_min_rtt / 1000.0);

        if (s->bbr_info->bbr_pacing_gain) {
            xo_emit(",{Lc:pacing_gain}{:pacing-gain/%g}",
            (double)s->bbr_info->bbr_pacing_gain / 256.0);
        }

        if (s->bbr_info->bbr_cwnd_gain) {
            xo_emit(",{Lc:cwnd_gain}{:cwnd-gain/%g}", (double)s->bbr_info->bbr_cwnd_gain / 256.0);
        }

        xo_emit(")");

        xo_close_container("bbr-info");
    }

    if (s->send_bps) {
        xo_emit("{P: }{Lw:send}{:send}{N:bps}", sprint_bw(b1, s->send_bps));
    }

    if (s->lastsnd) {
        xo_emit("{P: }{Lc:lastsnd}{:time-since-last-packet-sent-ms/%u}", s->lastsnd);
    }

    if (s->lastrcv) {
        xo_emit("{P: }{Lc:lastrcv}{:time-since-last-packet-recv-ms/%u}", s->lastrcv);
    }

    if (s->lastack) {
        xo_emit("{P: }{Lc:lastack}{:time-since-last-ack-recv-ms/%u}", s->lastack);
    }

    if (s->pacing_rate) {
        xo_emit("{P: }{Lw:pacing_rate}{:pacing-rate}{N:bps}", sprint_bw(b1, s->pacing_rate));

        if (s->pacing_rate_max) {
            xo_emit("/{:pacing-rate-max}{N:bps}", sprint_bw(b1, s->pacing_rate_max));
        }
    }

    if (s->delivery_rate) {
        xo_emit("{P: }{Lw:delivery_rate}{:delivery-rate}{N:bps}", sprint_bw(b1, s->delivery_rate));
    }

    if (s->app_limited) {
        xo_emit("{P: }{L:app_limited}{en:app-limited/true}");
    }

    if (s->busy_time) {
        xo_emit("{P: }{Lc:busy}{:busy-msec/%llu}{N:ms}", s->busy_time / 1000);

        if (s->rwnd_limited) {
            xo_emit(
                "{P: }{Lc:rwnd_limited}{:rwnd-limited-ms/%llu}{N:ms}({:rwnd-limited-perc/%.1f})",
                s->rwnd_limited / 1000,
            100.0 * s->rwnd_limited / s->busy_time);
        }

        if (s->sndbuf_limited) {
            xo_emit("{P: }{Lc:sndbuf_limited}{:sndbuf-limited-ms/%llu}{N:ms}({:limited-perc/%.1f})",
                s->sndbuf_limited / 1000, 100.0 * s->sndbuf_limited / s->busy_time);
        }
    }

    if (s->unacked) {
        xo_emit("{P: }{Lc:unacked}{:unacked/%u}", s->unacked);
    }

    if (s->retrans || s->retrans_total) {
        xo_emit("{P: }{Lc:retrans}{:retrans/%u}/{:retrans-total/%u}", s->retrans, s->retrans_total);
    }

    if (s->lost) {
        xo_emit("{P: }{Lc:lost}{:lost/%u}", s->lost);
    }

    if (s->sacked && s->ss.state != SS_LISTEN) {
        xo_emit(" sacked:{:sacked/%u}", s->sacked);
    }

    if (s->fackets) {
        xo_emit(" fackets:{:fackets/%u}", s->fackets);
    }

    if (s->reordering != 3) {
        xo_emit(" reordering:{:re-ordering/%d}", s->reordering);
    }

    if (s->rcv_rtt) {
        xo_emit(" rcv_rtt:{:rcv-rtt/%g}", s->rcv_rtt);
    }

    if (s->rcv_space) {
        xo_emit(" rcv_space:{:rcv-space/%d}", s->rcv_space);
    }

    if (s->rcv_ssthresh) {
        xo_emit(" rcv_ssthresh:{:rcv-ssthresh/%u}", s->rcv_ssthresh);
    }

    if (s->not_sent) {
        xo_emit(" notsent:{:not-sent/%u}", s->not_sent);
    }

    if (s->min_rtt) {
        xo_emit(" minrtt:{:min-rtt/%g}", s->min_rtt);
    }
} /* tcp_stats_print */

static void
tcp_timer_print(struct tcpstat * s)
{
    ...
    if (s->timer) {
        ...
        xo_emit("{Lc: timer}{D:(}{:timer-name},{:expire-time},{:retrans/%d}{D:)}",
            tmr_name[s->timer], print_ms_timer(s->timeout), s->retrans);
    }
}

static void
sctp_timer_print(struct tcpstat * s)
{
    if (s->timer) {
        xo_emit("{P: }{Lc:timer}(T3_RX,{:timeout-ms},{:retrans/%d}",
...
tcp_show_line(char * line, const struct filter * f, int family)
{
    ...
    if (show_details) {
        ...
        if (opt[0]) {
            xo_emit("{P: }{Lc:opt}{q:sock-opt}", opt);
        }
    }
...
static void
print_skmeminfo(struct rtattr * tb[], int attrtype)
{
    ...
    if (!tb[attrtype]) {
        if (attrtype == INET_DIAG_SKMEMINFO) {
            ...
            xo_emit(
                "{P: }{Lc:mem}{D:(}{L:r}{:rmem/%u},{L:w}{:wmem/%u},{L:f}{:fmem/%u},{L:t}{:tmem/%u}{D:)}",
                minfo->idiag_rmem,
                minfo->idiag_wmem,
                minfo->idiag_fmem,
                minfo->idiag_tmem);
    ...
    xo_emit(
        "{P: }{Lc:skmem}{D:(}{L:r}{:rmem-alloc/%u},{L:rb}{:rcvbuf/%u},{L:t}{:wmem-alloc/%u},{L:tb}{:sndbuf/%u},{L:f}{:fwd-alloc/%u},{L:w}{:wmem-queued/%u},{L:o}{:optmem/%u}",
    ...
    if (RTA_PAYLOAD(tb[attrtype]) >=
        ...
        xo_emit(",bl{:backlog/%u}", skmeminfo[SK_MEMINFO_BACKLOG]);
    }

    if (RTA_PAYLOAD(tb[attrtype]) >=
        (SK_MEMINFO_DROPS + 1) * sizeof(__u32)) {
        /*out(",d%u", skmeminfo[SK_MEMINFO_DROPS]); */
        xo_emit(",d{:drops/%u}", skmeminfo[SK_MEMINFO_DROPS]);
    }
...
static void
print_md5sig(struct tcp_diag_md5sig * sig)
{
    out("%s/%d=",
...
static void
tcp_show_info(const struct nlmsghdr * nlh, struct inet_diag_msg * r,
              struct rtattr * tb[])
{
    double rtt       = 0;
    struct tcpstat s = {};
...
static const char *
format_host_sa(struct sockaddr_storage * sa)
{
    union {
...
static void
sctp_show_info(const struct nlmsghdr * nlh, struct inet_diag_msg * r,
               struct rtattr * tb[])
{
    ...
    if (tb[INET_DIAG_LOCALS]) {
        ...
        xo_emit("locals:{:locals-host}", format_host_sa(sa));

        for (sa++, len -= sizeof(*sa); len > 0; sa++, len -= sizeof(*sa)) {
            xo_emit(",{l:host-sa}", format_host_sa(sa));
        }
    }
...
static void
parse_diag_msg(struct nlmsghdr * nlh, struct sockstat * s)
{
...
static int
inet_show_sock(struct nlmsghdr * nlh,
               struct sockstat * s)
{
    struct rtattr * tb[INET_DIAG_MAX + 1];
    struct inet_diag_msg * r = NLMSG_DATA(nlh);
...
#define SEND_SHUTDOWN   2
    ...
    if (show_details) {
        ...
        {
            /*out(" v6only:%u", v6only); */
            xo_emit("{P: }{Lc:v6only}{:v6-only/%u}", v6only);
        }
...
static int
tcpdiag_send(int fd, int protocol, struct filter * f)
{
...
static int
sockdiag_send(int family, int fd, int protocol, struct filter * f)
{
    struct sockaddr_nl nladdr = { .nl_family = AF_NETLINK };
...
static int
kill_inet_sock(struct nlmsghdr * h, void * arg, struct sockstat * s)
{
...
static int
show_one_inet_sock(const struct sockaddr_nl * addr,
...
static int
inet_show_netlink(struct filter * f, FILE * dump_fp, int protocol)
{
    int err = 0;
    ...
    xo_open_instance(proto_name(protocol));
    xo_open_list("socket");
again:
...
Exit:
    ...
    xo_emit("\n");
    xo_close_instance(proto_name(protocol));
    rtnl_close(&rth);
    if (arg.rth) {
        rtnl_close(arg.rth);
    }
...
static int
tcp_show_netlink_file(struct filter * f)
{
    FILE * fp;
    char buf[16384];
    int err = -1;
    ...
    fclose(fp);
    return err;
} /* tcp_show_netlink_file */

static int
tcp_show(struct filter * f)
{
    FILE * fp   = NULL;
    char * buf  = NULL;
    int bufsize = 64 * 1024;
...
static int
dgram_show_line(char * line, const struct filter * f, int family)
{
    struct sockstat s = {};
    char * loc, * rem, * data;
    char opt[256];
...
static void
unix_list_drop_first(struct sockstat ** list)
{
...
static bool
unix_type_skip(struct sockstat * s, struct filter * f)
{
...
static void
unix_stats_print(struct sockstat * s, struct filter * f)
{
    char port_name[30] = {};
...
static int
unix_show_sock(const struct sockaddr_nl * addr, struct nlmsghdr * nlh,
               void * arg)
{
...
static int
handle_netlink_request(struct filter * f, struct nlmsghdr * req,
...
static int
packet_stats_print(struct sockstat * s, const struct filter * f)
{
...
static int
packet_show_sock(const struct sockaddr_nl * addr,
...
static int
packet_show_line(char * buf, const struct filter * f, int fam)
{
    unsigned long long sk;
    struct sockstat stat = {};
...
static int
netlink_show_one(struct filter * f,
...
static int
netlink_show_sock(const struct sockaddr_nl * addr,
                  struct nlmsghdr * nlh, void * arg)
{
    struct filter * f = (struct filter *)arg;
...
static bool
vsock_type_skip(struct sockstat * s, struct filter * f)
{
...
static void
vsock_addr_print(inet_prefix * a, __u32 port)
{
...
static void
vsock_stats_print(struct sockstat * s, struct filter * f)
{
    sock_state_print(s);
...
static int
vsock_show_sock(const struct sockaddr_nl * addr,
                struct nlmsghdr * nlh, void * arg)
{
    struct filter * f         = (struct filter *)arg;
...
static void
tipc_sock_addr_print(struct rtattr * net_addr, struct rtattr * id)
{
...
static int
tipc_show_sock(const struct sockaddr_nl * addr, struct nlmsghdr * nlh,
               void * arg)
{
...
static int
generic_show_sock(const struct sockaddr_nl * addr,
                  struct nlmsghdr * nlh, void * arg)
{
...
static int
handle_follow_request(struct filter * f)
{
    int ret = 0;
    int groups = 0;
    struct rtnl_handle rth, rth2;
...
static int
get_snmp_int(char * proto, char * key, int * result)
{
    char buf[1024];
    FILE * fp;
...
static void
get_sockstat_line(char * line, struct ssummary * s)
{
    char id[256], rem[256];
...
static int
get_sockstat(struct ssummary * s)
{
    char buf[256];
    FILE * fp;
...
//    xo_open_container("ss");
    ...
    {
        xo_open_list("netlink");
        netlink_show(&current_filter);
        xo_close_list("netlink");
    }
